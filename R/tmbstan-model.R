...
fit_tmbstan <- function(tmb_input,
  ...
  if(use_inits)
    init <- replicate(chains, tmb_input$par_init, simplify = FALSE)
  else
    init <- "random"
  
  f <- tmbstan::tmbstan(fit0$obj, chains = chains, iter = iterations, init = init,
                        refresh = refresh, seed = rng_seed, cores = cores,
                        laplace = laplace)
  fit <- list(stanfit = f, tmbobj = fit0$obj)
  class(fit) <- "tmb_stanfit"

  fit
}

#' @export
sample_tmbstan <- function(tmb_stanfit, verbose = FALSE) {

  stopifnot(methods::is(tmb_stanfit, "tmb_stanfit"))

  smp <- as.matrix(tmb_stanfit$stanfit, pars = "lp__", include = FALSE)
  obj <- tmb_stanfit$tmbobj
